...
def main():
    print "Usage: test.py conf.gro topol.top grompp.mdp"
    ...
    CompMap_GMX = {'Bond':'Bond', 'Angle':'Angle','Proper-Dih.':'Dihedral',
                   'Improper-Dih.':'Dihedral', 'LJ-14':'Nonbonded',
                   'Coulomb-14':'Nonbonded', 'LJ-(SR)':'Nonbonded',
                   'Disper.-corr.':'Nonbonded', 'Coulomb-(SR)':'Nonbonded',
                   'Coul.-recip.':'Nonbonded'}

    # CompMap_OMM = {'HarmonicBondForce':'Bond',
    #                'HarmonicAngleForce':'Angle',
    #                'PeriodicTorsionForce':'Dihedral',
    #                'NonbondedForce':'Nonbonded'}
    # Now do the comparisons
    print('%-20s | %-15s | %-15s' % ('Component', 'Gromacs', 'OpenMM'))
    print('-'*56)
    total = 0
    for name, oe in Ecomps_OMM.items():
        oe / 4.184
        if name == 'HarmonicBondForce':
            term, ge = 'Bond', sum([Ecomps_GMX.get(k,0.0) for k, v in CompMap_GMX.items() if v == 'Bond'])
        elif name == 'HarmonicAngleForce':
            term, ge = 'Angle', sum([Ecomps_GMX.get(k,0.0) for k, v in CompMap_GMX.items() if v == 'Angle'])
        elif name == 'PeriodicTorsionForce':
            term, ge = 'Dihedral', sum([Ecomps_GMX.get(k,0.0) for k, v in CompMap_GMX.items() if v == 'Dihedral'])
        elif name == 'NonbondedForce':
            term, ge = 'Nonbonded', sum([Ecomps_GMX.get(k,0.0) for k, v in CompMap_GMX.items() if v == 'Nonbonded'])
        else:
            continue
        ge / 4.184
        total += oe
        print('%-20s | %15.6f | %15.6f' % (term, ge, oe))
    print('%-20s | %15.6f | %15.6f' % ('Total', Ecomps_GMX['Potential'], total))
    print('-'*56)
    print('-'*56)
    ...
    print('-'*56)
    print('Projection of Gromacs and OpenMM force:')
    print('-'*56)
